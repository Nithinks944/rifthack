function Timeline({ timeline }) {
  return (
    <section className="glass-card rounded-2xl p-5">
      <h2 className="mb-4 text-lg font-semibold text-white">CI/CD Status Timeline</h2>
      <ol className="space-y-3">
        {timeline.length === 0 ? (
          <li className="text-sm text-slate-400">No pipeline activity yet.</li>
        ) : (
          timeline.map((step, index) => (
            <li key={`${step.iteration}-${index}`} className="rounded-lg border border-slate-700/50 bg-slate-900/40 p-3">
              <div className="flex items-center justify-between">
                <span className="font-medium text-slate-100">{step.iteration}/{step.maxRetries}</span>
